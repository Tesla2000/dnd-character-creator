...
RACE_SUBRACE_PAIRS = tuple(
    (race, subrace) for race in Race for subrace in _get_subraces(race)
)
RACE_SUBRACE_IDS = [
...
class TestSimplifiedBuilder:
    ...
    def test_custom_blocks_skip_auto_population(self):
        ...
        custom_blocks = (WizardLevelIncrementer(),)
    ...
    def test_invalid_level_incrementer_length(self):
        ...
        with pytest.raises(ValidationError):
            SimplifiedBlocks(
                classes=classes,
                level_incrementers=(WizardLevelIncrementer(),),
            )
    ...
    def test_make_random_spell_assigner_unsupported_class_raises(self) -> None:
        with pytest.raises(NotImplementedError):
            _make_random_spell_assigner(Class.FIGHTER)
